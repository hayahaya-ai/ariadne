...
func renderCaseCompareClosureReceiptsDashboard(w io.Writer, receipts []model.CaseCompareClosureReceipt) {
	...
	for _, receipt := range receipts {
		...
		fmt.Fprintf(w, `<td><h3>Artifacts</h3>%s<h3>Artifact hashes</h3>%s<h3>Verification commands</h3>%s<h3>Remaining action</h3>%s<h3>Compare commands</h3>%s</td>`, renderSmallList(receipt.ArtifactSources), renderSmallList(caseCompareArtifactHashLines(receipt.ArtifactHashes)), renderCommandList(limitStrings(receipt.VerificationCommands, 2)), renderSmallList(nonEmptyStrings(receipt.RemainingAction)), renderCommandList(limitStrings(receipt.CompareCommands, 2)))
		fmt.Fprintln(w, "</tr>")
	}
	fmt.Fprintln(w, "</tbody></table></div>")
	fmt.Fprintln(w, "</section>")
