...
Compare output is the rerun readout. Use `ariadne compare --before before-proof.json --after after-proof.json` after saving JSON from `proofs`, `cases`, or `assess`. It reports whether each case closed, reopened, stayed open, stayed closed, changed, was added, or was removed, plus before/after control evidence, proof patch counts, evidence reference counts, source-backed evidence-reference details, evidence deltas, and next steps.
