...
Proofs output is the narrower closure-loop action view. Use `ariadne proofs --path . --case case:input-trust-boundary` when the operator already knows which case to work and needs only the evidence references, proof patches, parser-recognized fields, rerun commands, success criteria, and limitations. Use `--format html` for a focused proof-plan dashboard. It is read-only: proof patches are evidence plans, not automatic remediation or proof of live enforcement.
