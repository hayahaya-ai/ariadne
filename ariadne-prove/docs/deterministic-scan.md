...
For GitHub Actions managed-agent workflows, Ariadne separates workflow evidence into typed facts instead of treating the workflow as a generic script. Pull request, issue, workflow-run, or repository-dispatch triggers become `trustinput:managed-workflow-trigger`; AI or agent-like workflow steps become `tool:managed-agent-workflow`; repository write permissions become `authority:repository-write` reaching `boundary:repository-integrity-boundary`; `id-token: write` becomes `authority:cloud-identity-token` reaching `boundary:cloud-identity-boundary`; CI secret references become `authority:credential-access` reaching `boundary:ci-secret-boundary`; external actions, package calls, APIs, or web calls become `authority:external-communication`. Ariadne never runs the workflow and never emits secret values.
