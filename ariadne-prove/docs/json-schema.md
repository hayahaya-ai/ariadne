...
- `source_reference_workbench`, the structured evidence-opening contract used by the dashboard: ranked source refs, row-level source labels, line labels, facts, local paths when available, metadata-only flags, and inspect commands. Private, history, transcript, paste, and cache surfaces use metadata-only commands by default.
...
- `operator_workbench`, the structured operator workspace behind the dashboard, including `signal_chain`, `evidence_to_open`, `proof_state`, and `closure_loop`. The closure loop is the stable before -> change -> rerun -> after -> compare -> closure-decision contract for proving whether a case closed or stayed open.
